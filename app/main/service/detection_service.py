...
from app.main.model.result import Result, ShotResult
...
def convert_results_to_coordinates(results, model):
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            b = box.xyxy[0]
            c = box.cls
            coordinates.append((model.names[int(c)], int(
                c), list(map(int, b.tolist())), str(conf)))
...
def detect_target_boxes_array(images):
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            c = box.cls
            coordinates.append(
                (model.names[int(c)], int(c), b.tolist(), str(conf)))
...
def detect_target_image_cropping(pil_images) -> Tuple[Dict[str, str], int]:
    ...
    coordinate_results = detect_target_boxes_array(
        [t[0] for t in cropped_image_results])
    ...
    for x, coordinate_result in enumerate(coordinate_results):
        ...
        annotator = Annotator(np.ascontiguousarray(
            original_image), font='Arial.ttf')
        ...
        definitive_results.append(
            (definitive_coordinates, convert_to_base64(image)))
...
def detect_target_and_null_scores(result, model, image, scores, annotator):
    ...
    for box in boxes:
        ...
        if model.names[int(c)] == '0':
            scores.append(shootingScore(
                model.names[int(c)], int(c), b.tolist(), str(confidence)))
    ...
    for x, coordinate in enumerate(coordinates):
        ...
        if name != 'black_contour' and name != 'Target':
            ...
            for y, compare_coordinate in enumerate(coordinates):
                ...
                overlap_percentage, detection_size = check_overlap(
                    coordinate[2], compare_coordinate[2])

                # saves the overlap perecentage, the detected size and the id of the score and the compared y
                overlap_2d_array.append(
                    (overlap_percentage, detection_size, newScore.id, y, x))
...
def detect_target_single_image_cropping(image) -> shootingCard:
    ...
    original_image, cropping, scores, highest_target_confidence, annotator = detect_target_and_null_scores(
        results[0], model, image, scores, annotator)
    ...
    overlap_2d_array, distinct_areas, scores = fill_2d_array_and_scores(
        coordinates, cropping, cropping_detection, overlap_2d_array, distinct_areas, scores)
